...
MultiWAN NFT stores routing decisions in `multiwan-nft.globals.mmx_mask`,
defaulting to `0x3F0000`. The mask must be a 32-bit hexadecimal value, contain
at least three set bits, and avoid the lower byte `0x000000ff`, which is
reserved for MultiWAN QoS.

Safe examples include `0x3F0000` and `0x00FC0000`. After changing the mask,
restart both services so routing rules, cached status, and QoS connection
display use the same layout:
