...
load_nft_set_families() {
    MULTIWAN_QOS_SET_FAMILIES=""

    collect_set_family() {
        local section="$1" name enabled family
        ...
        MULTIWAN_QOS_SET_FAMILIES="${MULTIWAN_QOS_SET_FAMILIES}${name} ${family}${_NL_}"
    }

    config_foreach collect_set_family ipset
}
...
generate_main_nft_file() {
    ...
    local dscptag_tmp validate_log
    dscptag_tmp="$(mktemp /tmp/multiwan_qos_dscptag.XXXXXX)" || {
        error_out "Failed to create nftables temp file."
        return 1
    }
    validate_log="$(mktemp /tmp/multiwan_qos_dscptag_validate.XXXXXX)" || {
        rm -f "$dscptag_tmp"
        error_out "Failed to create nftables validation log."
        return 1
    }
    MULTIWAN_QOS_DSCPTAG_TMP="$dscptag_tmp"
    MULTIWAN_QOS_DSCPTAG_VALIDATE_LOG="$validate_log"
    cleanup_dscptag_generation() {
        rm -f "$MULTIWAN_QOS_DSCPTAG_TMP" "$MULTIWAN_QOS_DSCPTAG_VALIDATE_LOG"
    }
    trap cleanup_dscptag_generation INT TERM HUP

cat << DSCPEOF > "$dscptag_tmp"
...
DSCPEOF
    ...
    if nft -c -f "$dscptag_tmp" >"$validate_log" 2>&1; then
        mv "$dscptag_tmp" /usr/share/nftables.d/ruleset-post/dscptag.nft
        rm -f "$validate_log"
        MULTIWAN_QOS_DSCPTAG_TMP=""
        MULTIWAN_QOS_DSCPTAG_VALIDATE_LOG=""
        return 0
    fi
