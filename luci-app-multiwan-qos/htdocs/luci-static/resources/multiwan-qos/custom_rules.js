...
return view.extend({
    handleSaveApply: function(ev) {
        return this.handleSave(ev)
            ...
            .then(() => {
                return callInitAction('multiwan-qos', 'restart');
            })
            .then(() => {
                ui.addNotification(null, E('p', _('All rules have been saved and applied.')), 'success');
            })
            .catch((err) => {
                ui.addNotification(null, E('p', _('Failed to save settings or restart MultiWAN QoS: ') + err.message), 'error');
            });
    },

    load: function() {
        return Promise.all([
            fs.read('/etc/multiwan-qos.d/custom_rules.nft')
                .then(content => {
                    content = content.replace(/^table\s+inet\s+multiwan_qos_custom\s*{/, '');
            ...
            fs.read('/etc/multiwan-qos.d/inline_dscptag.nft')
                .catch(err => {
                    console.error('Failed to read inline rules:', err);
                    return '';
                }),
            fs.read('/tmp/multiwan_qos_custom_rules_validation.txt')
                .catch(err => {
                    console.error('Failed to read validation result:', err);
                    return '';
                })
        ]);
    },
    ...
    render: function([customRules, inlineRules, validationResult]) {
        ...
        s = m.section(form.NamedSection, 'custom_rules', 'custom_rules', _('Custom Rules'));
        s.anonymous = true;
        s.addremove = false;
                ...
                E('div', { 'class': 'right' }, [
                    ...
                    E('button', {
                        ...
                        'click': ui.createHandlerFn(this, function() {
                            var customTextarea = document.querySelector('textarea[name="cbid.multiwan_qos.custom_rules.custom_rules"]');
                            if (customTextarea) {
                                customTextarea.value = '';
                            }
                            
                            var inlineTextarea = document.querySelector('textarea[name="cbid.multiwan_qos.custom_rules.inline_rules"]');
                            if (inlineTextarea) {
                                inlineTextarea.value = '';
                            }
                            
                            return fs.remove('/etc/multiwan-qos.d/custom_rules.nft')
                                .then(() => {
                                    return fs.remove('/etc/multiwan-qos.d/inline_dscptag.nft');
                                })
                                .then(() => {
                                    return ui.changes.apply();
                                })
        ...
        o.monospace = true;
        o.datatype = 'string';
        o.description = _('Enter your custom nftables rules here. The "table inet multiwan_qos_custom { ... }" wrapper will be added automatically.') +
        ...
        o.write = function(section_id, formvalue) {
            ...
            const newRules = `table inet multiwan_qos_custom {
${formvalue.trim()}
}`;

            // Delete the existing table before applying new rules
            // If deletion fails (table doesn't exist), ignore the error
            return fs.exec('nft', ['delete', 'table', 'inet', 'multiwan_qos_custom'])
                .catch(() => { /* ignore deletion error */ })
                .then(() => fs.write('/etc/multiwan-qos.d/custom_rules.nft', newRules))
                .then(() => fs.exec('/etc/init.d/multiwan-qos', ['validate_custom_rules']))
                .then(() => fs.read('/tmp/multiwan_qos_custom_rules_validation.txt'))
                .then((result) => {
        ...
        o.write = function(section_id, formvalue) {
            return fs.write('/etc/multiwan-qos.d/inline_dscptag.nft', formvalue || '')
                .then(() => fs.exec('/etc/init.d/multiwan-qos', ['validate_custom_rules']))
                .then(() => fs.read('/tmp/multiwan_qos_custom_rules_validation.txt'))
                .then((result) => {
                    if (result.includes('Overall validation: PASSED')) {
        ...
        o.onclick = function(ev) {
            ...
            var customRulesTextarea = document.getElementById('widget.cbid.multiwan_qos.' + section_id + '.custom_rules');
            var inlineRulesTextarea = document.getElementById('widget.cbid.multiwan_qos.' + section_id + '.inline_rules');
            ...
            var fullCustomContent = `table inet multiwan_qos_custom {\n${currentCustomRules.trim()}\n}`;
        
            ui.showModal(_('Validating Rules'), [
                E('p', { 'class': 'spinning' }, _('Please wait while the rules are being validated...'))
            ]);
        
            return fs.write('/etc/multiwan-qos.d/custom_rules.nft', fullCustomContent)
                .then(() => {
                    return fs.write('/etc/multiwan-qos.d/inline_dscptag.nft', currentInlineRules || '');
                })
                .then(() => {
                    return fs.exec('/etc/init.d/multiwan-qos', ['validate_custom_rules']);
                })
                .then(() => {
                    return fs.read('/tmp/multiwan_qos_custom_rules_validation.txt');
                })
                .then((result) => {
                    ui.hideModal();
                    if (result.includes('Overall validation: PASSED')) {
