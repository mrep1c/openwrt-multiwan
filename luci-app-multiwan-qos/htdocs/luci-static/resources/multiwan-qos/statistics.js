...
// Define RPC calls
var callMultiWANQoSStats = rpc.declare({
...
return view.extend({
    ...
    createPollFunction: function() {
        ...
        return function() {
            return Promise.all([
                ...
                var contentContainer = document.querySelector('.multiwan_qos-content-container');
                if (contentContainer) {
                    // Show error message
                    dom.content(contentContainer, E('div', { 'class': 'cbi-section' }, [
                        E('p', { 'class': 'alert-message warning' }, [
                            _('Error loading QoS statistics. Please check if the MultiWAN QoS service is running.')
                        ])
                    ]));
                }
    ...
    render: function(data) {
        ...
        var existingSettings = document.querySelector('.multiwan_qos-settings-section');
        ...
        if (!existingContainer) {
            ...
            var settingsSection = E('div', { 'class': 'cbi-section multiwan_qos-settings-section', 'style': 'margin-bottom: 5px;' });
            
            // Create header for settings
            var settingsHeader = E('h3', {}, _('MultiWAN QoS Settings'));
            ...
            var contentContainer = E('div', { 'class': 'multiwan_qos-content-container' });
        ...
        if (qosStats) {
            ...
            if (qosStats.root_qdisc === 'cake') {
                ...
                if (qosStats.strict_realtime_priority !== undefined) {
                    ...
                    row.appendChild(document.createTextNode(' '));
                    row.appendChild(E('span', {}, qosStats.strict_realtime_priority
                        ? (qosStats.realtime_rate_mode === 'adaptive' ? _('Ignored by Adaptive') : _('Enabled'))
                        : _('Disabled')));
                    infoContainer.appendChild(row);
                }
            ...
            contentContainer.appendChild(E('div', { 'class': 'cbi-section' }, [
                E('p', {}, _('No QoS statistics available. Make sure MultiWAN QoS is running.'))
            ]));
        }
    },
