...
return view.extend({
    ...
    render: function() {
        ...
        o = s.option(form.ListValue, 'realtime_rate_mode', _('Realtime Rate Mode'),
            _('Auto uses a fixed 1500 kbit/s reserve capped at 25% of the link. Manual uses the overrides below. Adaptive idles and starts new realtime sessions at 1000 kbit/s. While realtime traffic is active, measured demand may adjust the HFSC rate from 300 to 1800 kbit/s, capped at 25% of the link. After 20 seconds without any realtime packets, the rate returns to 1000 kbit/s. Increases use the highest one-second demand sample from the last 3 seconds plus a 200 kbit/s reserve. Decreases use 30-second smoothed demand and 5-second burst memory, require clean drop and backlog history, and move in 50 kbit/s steps no faster than every 10 seconds. BFIFO and PFIFO use a fixed queue profile calculated at 1000 kbit/s; Adaptive changes only the HFSC class and never resizes the selected game qdisc.'));
        o.value('auto', _('Auto'));
