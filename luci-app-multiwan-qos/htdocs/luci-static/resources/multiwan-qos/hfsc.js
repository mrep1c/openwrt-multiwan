...
return view.extend({
    ...
    render: function() {
        ...
        o = s.option(form.ListValue, 'realtime_rate_mode', _('Realtime Rate Mode'),
            _('Auto uses a fixed 1500 kbit/s reserve capped at 25% of the link. Manual uses the overrides below. Adaptive starts at 1000 kbit/s and adjusts only the HFSC realtime service rate from 300 to 1800 kbit/s, capped at 25% of the link. Increases use the highest one-second demand sample from the last 3 seconds plus a 200 kbit/s reserve. Decreases use 30-second smoothed demand and 5-second burst memory, require clean drop and backlog history, and move in 50 kbit/s steps no faster than every 10 seconds. Traffic returning after an extended idle starts at no less than 1000 kbit/s. The selected game queue is never rebuilt or resized.'));
        o.value('auto', _('Auto'));
