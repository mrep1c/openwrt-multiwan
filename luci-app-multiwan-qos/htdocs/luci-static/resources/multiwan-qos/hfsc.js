...
return view.extend({
    ...
    render: function() {
        ...
        o = s.option(form.ListValue, 'realtime_rate_mode', _('Realtime Rate Mode'),
            _('Auto uses a fixed 1500 kbit/s reserve capped at 25% of the link. Manual uses the overrides below. Adaptive starts the HFSC realtime rate at 1000 kbit/s and adjusts its sustained service rate from 300 to 1800 kbit/s, capped at 25% of the link. Increases use the highest one-second demand sample from the last 3 seconds plus a 200 kbit/s reserve. Decreases use 30-second smoothed demand and 5-second burst memory, require clean drop and backlog history, and move in 50 kbit/s steps no faster than every 10 seconds. FIFO/PFIFO burst profiles start at 1500 kbit/s and adjust independently from 1500 to 2000 kbit/s after burst drops; other selected game leaves remain static. New PC-agent flow sessions and traffic returning after an extended idle start at no less than 1000 kbit/s.'));
        o.value('auto', _('Auto'));
        o.value('manual', _('Manual'));
        o.value('adaptive', _('Adaptive'));
        o.default = 'auto';

        o = createOption('GAMEUP', _('Realtime Upload Reserve Override (kbit/s)'), _('Manual upload reserve. A blank value keeps the automatic reserve for this direction.'), _('Default: auto'), 'uinteger');
        o.depends('realtime_rate_mode', 'manual');
        o = createOption('GAMEDOWN', _('Realtime Download Reserve Override (kbit/s)'), _('Manual download reserve. A blank value keeps the automatic reserve for this direction.'), _('Default: auto'), 'uinteger');
        o.depends('realtime_rate_mode', 'manual');
