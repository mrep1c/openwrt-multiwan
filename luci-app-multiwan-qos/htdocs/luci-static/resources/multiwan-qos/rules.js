...
return view.extend({
        ...
        if (Array.isArray(ruleCounters)) {
            ruleCounters.forEach(function (rule) {
                view.counterData[rule.rule_name] = {
                    ...
                    in_packets: rule.in_packets || 0,
                    out_packets: rule.out_packets || 0,
                    in_bytes: rule.in_bytes || 0,
                    out_bytes: rule.out_bytes || 0,
            ...
            if (activityColumnIndex >= 0) {
                ...
                dataRows.forEach(function (row) {
                    ...
                    if (cells[activityColumnIndex] && sectionId) {
                        var ruleName = uci.get('multiwan-qos', sectionId, 'name');
                        var counterEnabled = uci.get('multiwan-qos', sectionId, 'counter');
    ...
    formatCounterDisplay: function (ruleName, counterEnabled) {
        ...
        var formatCount = function (pkts) {
            if (pkts >= 1000000) return (pkts / 1000000).toFixed(1) + 'M pkts';
            if (pkts >= 1000) return (pkts / 1000).toFixed(1) + 'K pkts';
            return pkts + ' pkts';
        };
        ...
        if (stats.ipv4_packets > 0 && stats.ipv6_packets > 0) {
            breakdown = `\n(Total v4:${stats.ipv4_packets}, v6:${stats.ipv6_packets})`;
        }

        return E('div', {
            'style': 'color: #0a84ff; font-size: 0.9em; font-weight: 500; line-height: 1.4;',
            'title': `Total: ${stats.packets} packets, ${formatBytes(stats.bytes)}${breakdown}`
        }, [
            E('div', {}, `Out: ${formatCount(stats.out_packets)} (${formatBytes(stats.out_bytes)})`),
            E('div', {}, `In: ${formatCount(stats.in_packets)} (${formatBytes(stats.in_bytes)})`)
        ]);
    },
    ...
    render: function () {
        ...
        m = new form.Map('multiwan-qos', _('MultiWAN QoS Rules'),
        ...
        o.cfgvalue = function (section_id) {
            var proto = uci.get('multiwan-qos', section_id, 'proto');
        ...
        o.cfgvalue = function (section_id) {
            var value = uci.get('multiwan-qos', section_id, 'proto');
        ...
        o.write = function (section_id, value) {
            if (value && value.length) {
                uci.set('multiwan-qos', section_id, 'proto', value.join(' '));
            } else {
                uci.unset('multiwan-qos', section_id, 'proto');
            }
        };
        ...
        o.write = function (section_id, formvalue) {
            ...
            uci.set('multiwan-qos', section_id, 'enabled', formvalue);
        };
        o.load = function (section_id) {
            var value = uci.get('multiwan-qos', section_id, 'enabled');
        ...
        o.cfgvalue = function (section_id) {
            var counterEnabled = uci.get('multiwan-qos', section_id, 'counter');
            if (counterEnabled !== '1') {
                return '-';
            }
            return 'loading...';
        ...
        return m.render().then(function (rendered) {
            ...
            document.multiwan_qosRulesView = self;
