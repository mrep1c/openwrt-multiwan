...
TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/multiwan-nft-lock.XXXXXX")" || exit 1
...
# Router lock operations must not fork cat(1) or date(1).
...
case "$lock_token" in
	"lock:$$:$self_start:"[0-9]*) ;;
	*) fail "lock token did not use the process identity" ;;
esac
...
if sh -c 'helper="$1"; lock_dir="$2"; set -- none; . "$helper"; mw_lock_acquire "$lock_dir"' \
	sh "$REPO_ROOT/multiwan-nft/files/lib/multiwan-nft/process-lock.sh" "$lock_dir"; then
...
printf 'NFT process-lock regression tests passed.\n'
